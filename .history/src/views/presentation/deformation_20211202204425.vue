<template>
  <div class="deformation">
    <div class="tool-wrapper">
      ...
      ></el-slider>
      <div class="confirm" @click="dialogVisible = true">
        {{ $t("exportCartoon") }}
      </div>
    </div>
    ...
    <el-dialog title="请选择导出类型" v-model="dialogVisible" width="20%">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="handleExportCartoon"
            >确 定</el-button
          >
        </span>
      </template>
...
<script>
import { querySensorData, exportGif } from "@/api/deformation"; //getFileData
  ...
  methods: {
    handleExportCartoon() {
      if (this.date && this.date.length) {
        ...
        let self = this;
        exportGif(this.params)
          .then((res) => {
            self.dialogVisible = false;
            window.open(
              window.customENV.baseURL +
                `common/download?fileName=${encodeURI(
                  res.filePath
                )}&delete=true`
            );
            loading.close();
    ...
    handleChange() {
      ...
      const timestampBegin = new Date(this.date[0]).getTime();
      const timestampEnd = new Date(this.date[1]).getTime();
      if (timestampEnd > timestampBegin + 3600 * 1000 * 24 * 9) {
        this.$message({
          showClose: true,
          message: `日期的起止时间跨度不能超过10天!`,
          type: "warning",
        });
      } else {
        this.getData();
    ...
    fileData(i) {
      ...
      // });
      self.$sendIframe("deformation", self.dataList[i].jsonPath);
    },
