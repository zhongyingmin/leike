        ...
        <div class="body">
          <div class="coll" v-for="(data, index) in tableData" :key="index">
            ...
            <div class="handle">
              ...
              <div class="edit" @click="resetPassWord(data)">
                {{ $t("resetPassword") }}
              </div>
              <div
                class="edit"
              ...
              <div class="remove" @click="handleRemove(data)">
                {{ $t("remove") }}
              </div>
            </div>
          </div>
    ...
    >
      <el-form ref="form" :model="form" :rules="formRules" label-width="100px">
        ...
        <el-form-item :label="$t('post')" required>
          <el-select v-model="form.postId" :placeholder="$t('please')">
            <el-option
              v-for="item in options"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item :label="$t('role')" required>
          <el-radio-group v-model="form.roleId">
            <el-radio :label="0">{{$t('administrator')}}</el-radio>
            <el-radio :label="1">{{$t('averageUser')}}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item :label="$t('sex')" required>
          <el-radio-group v-model="form.sex">
            <el-radio :label="'0'">{{$t('boy')}}</el-radio>
            <el-radio :label="'1'">{{$t('girl')}}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
      <template #footer>
  ...
  data() {
    ...
    return {
      ...
      formRules: {
        ...
        passwd: [{ trigger: "blur", validator: validatePassword }],
      },
      options: [
        {
          value: "1",
  ...
  methods: {
    ...
    confirm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (self.title == this.$t("createUser")) {
            self.handleCreate();
          } else {
            self.handleUpdate();
          }
