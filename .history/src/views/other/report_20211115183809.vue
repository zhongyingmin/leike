      ...
      <div class="form-wrapper">
        ...
        <div class="form-item">
          ...
          <div class="text-wrapper">
            ...
            <span style="color:red">生成失败请回到主页重新生成</span>
          </div>
        </div>
        <div class="form-item">
          ...
          <div class="text-wrapper">
            <div v-for="data in checkedArr" :key="data">
              <div :key="'valueimage_' + data" :ref="'valueimage_' + data"></div>
              <div :key="'speedimage_' + data" :ref="'speedimage_' + data"></div>
              <div
  ...
  methods: {
    ...
    initCharts() {
      ...
      this.checkedArr.forEach((fid) => {
        ...
        let chartChecked3 = echarts.init(self.$refs["accelerationimage_" + fid]);
        feature({ fids: fid }).then((res) => {
          self.$nextTick(() => {
            self.charts.push(chartChecked1);
            self.charts.push(chartChecked2);
            self.charts.push(chartChecked3);
            self.charts['valueimage_'+fid]=chartChecked3
            self.charts['speedimage_'+fid]=chartChecked3
